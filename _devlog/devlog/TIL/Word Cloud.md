...
`search.json`을 fetch해서 최근 1년 포스트만 필터링 → 각 포스트의 `tags` 배열을 모아 태그별 등장 횟수(`freq` 객체)를 셈 → 빈도 내림차순 정렬 후 상위 60개 추출 → `[["JavaScript", 12], ["Flutter", 8], ...]` 형태의 배열로 만들어 `wordcloud2.js`에 전달.
