...
- 나머지 한 제품 — 코드 저장소가 공개라 배포할 때마다 그쪽에서 버전 목록을 받아 온다. 새 버전을 내면 사이트가 알아서 따라가니 이쪽이 편해 보였다

문제는 편해 보이던 쪽에서 났다. 그 요청에는 시간당 횟수 제한이 있는데, 한도가 우리 계정이 아니라 요청이 나가는 서버 주소 기준이었다. 배포 서버는 여럿이 함께 쓰는 주소다.
