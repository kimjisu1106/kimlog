---
layout: post
title: 회사 툴 — 업무 관리 도구 개발 정리
date: 2026-06-12
description: 주간 업무 보고를 웹 도구로 옮기며 정한 것들 — 비밀번호는 한 시스템만 소유하고, 권한은 코드가 아니라 데이터로 둔다.
permalink: "mtaw39lz"
categories:
  - today-i-learn
...
`pip install anthropic`을 했더니 환경 문제로 구버전(0.2.10)이 깔리며 `urllib3`·`idna`를 다운그레이드해 인사 시스템 호출이 깨질 뻔했다. SDK를 버리고 HTTP로 직접 불렀다.
