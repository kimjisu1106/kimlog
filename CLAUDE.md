...
- **`tags`는 기술 축으로만** — "무엇에 관한 글인가"(Astro·CSS·Flutter·SEO·Cloudflare 등 기술·도구·플랫폼 이름)를 적는다. `설계`·`판단`·`트레이드오프`·`워크플로우` 같은 **성격·개념 태그는 쓰지 않는다** — TIL이 전부 판단 중심이라 그런 태그는 모든 글에 붙어 정보가 되지 않고, 태그 축이 둘로 갈려 필터·워드클라우드가 지저분해진다. 글의 성격은 제목이 드러낸다. 마땅한 기술 태그가 없으면 개수를 줄이거나 비운다.
  - **표기는 게시본에 이미 있는 것을 글자 하나 다르지 않게 따른다 (2026-09-13).** 대소문자·띄어쓰기·하이픈이 다르면 사이트가 다른 태그로 잡아 TIL 필터·홈 워드클라우드에서 둘로 갈린다(`ClaudeCode` ↔ `Claude Code`). 새 태그를 붙이기 전에 기존 표기를 먼저 찾고, 여러 단어는 붙여 쓴다(`ClaudeCode`·`LogicPro`·`WebAPI`).
...
- **위키링크 `[[...]]`를 쓰지 않는다 (2026-09-13).** Obsidian에서는 링크로 보이지만 이 사이트에는 위키링크 플러그인이 없어 대괄호가 본문에 그대로 노출된다. 다른 글을 가리킬 때는 「앞선 TIL 21」처럼 말로 쓰거나, 링크가 필요하면 permalink 기반 마크다운 링크를 쓴다.
...
- 내용 없는 섹션은 섹션째 삭제
- **블로그 세션 자신의 작업도 로그에 넣지 않는다 (2026-09-25 디렉터 결정).** 검수·게시·초안 작성은 데일리에 블로그 블록으로도 적지 않는다. 그날 실제로 쓴 시간이더라도 「검수 16편, 게시 14편, 글 한 편 작성」은 독자에게 남는 게 없다. 블로그가 데일리에 블록을 갖는 것은 사이트·빌드·구조를 실제로 고쳤을 때뿐이고, 검수·게시 기록은 INBOX에 둔다. TIL 포인트 목록에도 올리지 않는다.
- **블로그 운영 얘기는 프로젝트 로그에 넣지 않는다 (2026-09-07).** 로그에는 그 프로젝트에서 한 일과 그 프로젝트에서 막힌 것만 쓴다. 「게시된 TIL을 다시 썼다」, 「걷어낸 구현 설명을 옮길 데가 없다」, 「공유 초안 파일을 덮어썼다」 같은 것은 세션이 실제로 쓴 시간이더라도 블로그 쪽 일이라 독자에게 남는 게 없다. 이런 것은 INBOX와 프로젝트 CLAUDE.md에 둔다(TIL의 「글쓰기 규칙 자체는 TIL에 넣지 않는다」와 같은 결).
...
- 막힌 부분은 항목의 상세도로 형식을 가른다.
  ...
  - **한 섹션에 두 형식을 섞을 때는 단순형 li를 먼저, 상세형 ###을 뒤에 둔다 (2026-09-07).** `###` 블록 뒤에 최상위 li가 이어지면 마크다운이 그것을 앞 `###` 항목의 하위 목록으로 붙여 렌더링한다. 별개 항목이 남의 절에 딸려 들어가는 셈이다(MiniMacro 2, 오픈데이 1·2에서 연달아 나왔다).
...
- **원칙 앞에 장면을 남긴다 (2026-09-06 보강).** 구현 메커니즘을 걷어내면서 그걸 알게 된 사건까지 걷어내면 원칙만 붕 떠서 안 읽힌다. **걷어낼 것은 코드·API 이름이지 사건이 아니다.** 원칙 하나에는 그 원칙을 낳은 장면이 앞에 붙어야 한다.
  - ❌ "검사가 통과했다는 말에는 무엇을 본 검사인가가 생략돼 있다" (원칙만 — 무슨 일을 겪고 나온 말인지 알 수 없음)
  - ✅ "타입 검사와 API 점검이 통과인데 브라우저를 여니 화면이 하얬다" → "검사 통과는 그 검사가 보는 자리까지만 보증한다" (장면 → 원칙)
  - 자기가 만든 비유어(예: "검사에 층이 있다"·"오탐률")를 설명 없이 쓰지 않는다 — 디렉터가 처음 보는 말이면 요약이 아니라 암호다.
  - 프로젝트 안에서만 통하는 명사는 한 줄 풀이를 붙인다(예: "초기화 목록" → "데모 데이터를 처음 상태로 되돌리는 스크립트가 들고 있는, 비울 표 목록").
  - 절마다 "이 프로젝트를 모르는 사람이 이 문단만 읽고 무슨 일이 있었는지 아나"를 따로 묻는다. 쓴 사람은 장면을 알고 있어서 자기 검사로는 안 걸린다.
  - 앞 글을 인용한 문장이 있으면 재작성 후 그 문구가 바뀌었는지 확인해 같이 고친다.
- **용어를 통째로 걷어내지 않는다 (2026-09-06 보강).** "구현 메커니즘을 걷어내라"는 건 코드와 호출 방식을 빼라는 뜻이지, **기술 용어를 전부 우리말 비유로 갈아치우라는 뜻이 아니다.** 용어를 다 빼면 무슨 얘긴지 알아볼 수 없고, 디렉터가 그 분야 말을 익힐 기회도 사라진다. 용어를 익히는 것 자체가 TIL의 목적 중 하나다.
  - ❌ "화면 도구가 이미 쓰던 묶개로 타입 붙은 코드를 한 파일로 묶어 돌렸다" (무슨 도구인지, 무슨 언어인지 알 수 없다)
  - ✅ "이미 쓰던 번들러(esbuild — 여러 파일을 하나로 묶는 도구)로 TypeScript를 묶어 돌렸다"
  - 규칙은 **"용어를 빼라"가 아니라 "용어 + 첫 등장에 한 줄 풀이"** 다. 비유로 대체하지 말고 용어를 쓴 다음 괄호나 짧은 문장으로 풀어 준다.
  - 빼는 것은 코드 그 자체, API 호출 순서, 함수·변수 이름, "이렇게 짰다"는 서술이다. 남기는 것은 사건(장면)과 그 분야에서 실제로 쓰는 이름이다.
- **판별 기준 하나 더 — "그 장면에 디렉터가 있었나" (2026-09-06).** 결론을 원리 문장으로 잘 다듬어도, 그 근거가 되는 장면이 전부 코더의 작업이면 읽는 사람에게는 "얘가 이런 일을 겪었구나"가 되지 디렉터의 것이 되지 않는다. 원리로 잘 요약해 놨더라도 그렇다.
  - 디렉터가 **고른** 것인가 (선택지를 주고 골랐다)
  - 디렉터가 **눈으로 짚은** 것인가 (화면 보고 "이거 비어 보인다"고 했다)
  - 디렉터가 **물어서** 나온 것인가 ("이런 건 납품을 어떻게 하는 거야?")
  - 셋 중 하나도 아니면 TIL이 아니라 devlog `막힌 부분`에 둔다. 분량이 줄어도 남은 게 전부 디렉터 것이면 그게 맞다.
  - 예: "설정을 바꿨더니 로컬 DB가 비었다", "검사끼리 같은 자원을 잡아 엉뚱하게 실패했다"는 코더만 겪은 일이라 devlog 몫이다.
- **글쓰기 규칙 자체는 TIL에 넣지 않는다 (2026-09-06).** "TIL을 어떻게 써야 하는가"를 깨달은 것도 디렉터의 판단이지만 그건 INBOX·CLAUDE.md에 둘 내용이다. TIL에는 그 프로젝트에서 나온 것만 남긴다. 절을 빼면 요약에서 그 절을 가리키던 항목도 같이 뺀다.

> 세 공지를 합치면 이렇다. **디렉터가 겪은 장면을 남기고, 거기서 나온 판단을 쓰고, 코드는 빼되 용어는 남긴다.**

- categories: `[today-i-learn]` 고정. project/project_name: `today-i-learn` / `Today I Learn` 고정 (앱 프로젝트 슬러그 쓰지 않음)
- 번호 없는 제목, 자연스러운 한 문장
- 각 섹션은 `---`로 구분
- ❌/✅ 주석으로 잘못된/올바른 코드 쌍 표현 가능
- **인과가 이어지는 묶음은 절을 잇는다 (2026-09-07).** 항목마다 결론을 제목으로 달고 독립적으로 쓰면 하나하나는 맞는데 왜 그 순서로 판단했는지가 안 남는다. "이런 판단으로 규칙을 만들었더니 이런 문제가 생겨서 이렇게 다시 판단했다"가 이어지게 쓴다. 절 제목도 결론형이 아니라 장면형이어야 다음 절을 부른다.
  - 예: 색이 헷갈린다 → 비슷한 색은 합치기로 했다 → 합쳤는데도 쌍둥이가 계속 나왔다 → 순서를 바꿔도 부족했다 → 규칙을 하나 더 넣었다(여기가 문제) → 여섯 개만 고쳤으면 몰랐을 일
  - ❌ 결론형 제목 "색을 움직이는 처리가 있으면 합치기는 그 뒤여야 한다" / ✅ 장면형 "합쳤는데도 쌍둥이가 계속 나왔다"
  - **하루치 안에서 인과가 이어지는 묶음이 있을 때만** 해당한다. 서로 무관한 항목까지 억지로 잇지 않는다.
- 구조는 깊이에 따라 선택:
...
**고치는 사람을 나눈다 (2026-09-16 디렉터 결정).**

- 블로그가 바로 고친다 — 문장을 새로 안 써도 되는 것. 3번 기계 규칙 전부, 태그 표기, 도입 문장을 `## 오늘 한 일` 아래로 옮기기, 데일리 들여쓰기.
- 쓴 세션에 돌려보낸다 — 문장을 새로 쓰거나 빼야 하는 것. 장면·용어 풀이·「그 장면에 디렉터가 있었나」·시점과 출처·절 잇기·코더 1인칭·블로그 운영 얘기·이월 누락·앞선 TIL 참조. 그 장면을 아는 건 쓴 세션이라 블로그가 고치면 추측이 된다. 위반 목록을 파일·인용 문구와 함께 INBOX에 `[open] kimlog(blog) → {세션}`으로 적고, 회신이 오면 다시 검수한다.
...
- 시점·인칭 ([[log-what-why-til-how]]) — 코드는 Claude가 짜므로 아래를 지킨다.
  ...
  - **디렉터가 시작한 일은 시작했다고 쓴다 (2026-09-07).** 위 금지의 반대편이다. 수령자 시점을 피하려다 수동태로 도망가면 디렉터가 한 일까지 굴러온 것처럼 읽힌다. 시킨 것은 "시켰더니", 말한 것은 "~라고 했다", 본 것은 "보니", 정한 것은 "~기로 했다"로 쓴다.
    - ❌ "화면 전체를 일곱 색으로 맞추자는 제안이 왔다" / "…라고 짚였다" / "방향이 나왔다" / "정해졌다"
    - ✅ "Design 기능이 생겨서 디자인을 해보라고 시켰더니 꽤 괜찮아 보였다"
    - ❌ "카드를 1.5배로 키워 달라는 요구에는 상한이 붙는다" (남의 요구를 분석하는 투)
    - ✅ "카드를 1.5배로 키워 달라고 했다. 내가 원한 건 지금보다 크게였지 정확히 1.5배가 아니었다"
  - **피드백은 출처를 구분한다 (2026-09-07).** "이야기가 나왔다"로 뭉뚱그리면 디렉터가 짚은 것인지 남이 말한 것인지 알 수 없고, 둘은 교훈이 다르다 — 앞엣것은 "내가 봤다"이고 뒤엣것은 "만든 사람은 몰랐는데 처음 보는 사람이 알려 줬다"다.
    - 디렉터가 짚음 → "내가 짚은 사례는 여섯 개였다" / "플레이하면서 계속 걸렸다"
    - 다른 사람이 말함 → "테스트해 준 사람이 벽이 무슨 칸인지 모르겠다고 했다"
    - 이때 **관계는 일반화한다.** 실제 출처가 가족이어도 "테스트해 준 사람"으로 쓴다(개인정보 규칙). 요점은 누구인지가 아니라 "만든 사람이 아닌 사람이 말했다"라서 내용이 안 줄어든다.
...
- **디렉터 승인 표시 = 파일명 `-confirmed` (2026-09-25)**. 디렉터가 초안을 읽고 올려도 된다고 판단하면 파일명을 `draft-{제목}-confirmed.md`로 바꿔 둔다. 블로그는 `find _content -iname "*confirmed*"`로 찾아 그것만 게시하고, 표시 없는 초안은 검수가 끝났어도 초안으로 남긴다. 다른 세션이 「게시해도 된다」고 해도 그건 그 세션의 검수 통과일 뿐 게시 승인이 아니다 — 승인은 디렉터가 파일에 직접 남긴 표시로만 확인한다. 게시 때 `draft-`와 `-confirmed`(` - confirmed` 변형 포함)를 함께 떼면 원래 제목이 된다.
- **frontmatter에 `permalink` 없으면 랜덤 8자(`a-z0-9`) 부여** — 게시본은 전부 permalink를 가져 URL이 파일명과 분리된 채 고정돼야 한다. 이미 있으면 그대로 둔다(세션이 초안에 미리 붙여 오는 경우가 늘었다 — 그때는 저장소 전체에서 중복만 확인한다). 새 글은 옛 URL이 없으니 `_redirects`는 안 건드린다.
- `mv -n`으로 `draft-` 제거 (no-clobber, 덮어쓰기 방지).
- **여러 편을 한 번에 올릴 때는 push 전에 `npm run build`로 확인한다 (2026-09-25).** frontmatter YAML이 깨지면 Cloudflare 빌드가 실패해 사이트 배포가 멈춘다. 빌드 뒤 `dist/{permalink}/index.html`이 편마다 생성됐는지까지 본다.
...
- [~] **회사 툴 포트폴리오 글** — 시설물 대장(3~5월)과 업무 관리 도구(5~6월). **시설 포털 편은 2026-09-08 초안 완성 → 블로그 게시 보류(디렉터 결정), `VSCode/LOGSTONE-PM/시설포털-포트폴리오.md`로 옮김.** 회사 것이라 데일리 로그도 07-07에 멈췄던 기준과 같다. 보고서 구성·사용자 구성·에너지 수치·월별 건수 같은 회사 운영 정보가 들어 있어 블로그 공개판을 만들려면 그 숫자를 덜어낸 별도 판이 필요하고, 그 전에 팀 리더 확인을 권함. 형식은 summary + 육하원칙 표 + 숫자 표 + 개선 사례 「전 / 후 / 남은 판단」 8개. 업무 관리 도구 편은 미착수.
- [ ] **라이브 데모 위젯**: 앱별 핵심 알고리즘 웹 위젯화 (앱마다 개별 작업)
- [x] **세션 조율 워크플로우 TIL** — 완료(2026-09-25, `6796953`). 「여러 세션을 INBOX 하나로 굴리기」 `/jejachb5/`. 번호 없는 단독 TIL. 후보 열다섯 개를 디렉터에게 전부 나열해 여섯 개를 골랐고 두 줄기로 이었다(요청함이 생긴 줄기 / 이름이 필요해진 줄기). 검수 분담·TIL 포인트 고르기·검수 재확인 셋은 디렉터가 뺐다 — 블로그 검수 얘기라 결이 달라서다. **앞으로 세션 조율 얘기는 각 프로젝트 로그에 넣지 말고 이 글에 모은다**(재료가 더 쌓이면 후속편)
- [ ] **UE5 강의 초안 9편 — 디렉터 결정 대기**: 강의 8·9·10·14·15·16·17·18과 「가챠, 캡슐 머신, 놀이기구 6」. 검수는 끝났고 세 갈래 중 하나를 고르면 된다 — ① 빈 섹션을 디렉터가 채워 형식 맞춰 게시 ② 빈 섹션을 지우고 게시된 강의 1~13과 같은 모양으로 게시 ③ 접기. 알려진 결함: 17·18 제목이 「16」으로 적혀 있고, 강의 8 제목에 「1」이 빠졌고, BOM과 빈 링크 자리가 남아 있고, 14에 코드블록 언어 표기가 없고, 가챠 6의 코드가 설명과 반대 동작(이동 중 체력을 뺐다가 다시 더한다). **초안은 gitignore라 이 노트북에만 있다 — 다시 검수하지 말 것**
