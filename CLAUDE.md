...
app_url: "https://..." # (선택) 있으면 홈 Apps 섹션 노출 + 포스트 상단에 앱 카드 표시 (redirect_to는 jekyll-redirect-from 플러그인과 충돌하여 사용 금지)
---
```
...
Minima 기본 post layout을 오버라이드. 두 가지 기능이 자동으로 삽입됨:

**1. Redirect Card (상단)**
- `redirect_to`가 있는 포스트에만 표시
- `/assets/images/{project}.png` 썸네일 자동 참조
- 클릭 시 `redirect_to` URL로 새 탭 이동
