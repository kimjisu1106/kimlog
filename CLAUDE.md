...
- [ ] **세션 조율 워크플로우 TIL**: 여러 프로젝트 세션을 `LogStoneShop/INBOX.md`로 조율하는 방식을 독립 TIL/회고로 소개 (개인 워크플로우라 공개 안전 — 각 devlog에 조각으로 새는 대신 한 편으로). 2026-09-19 디렉터 결정으로 샵 TIL 3·7의 세션 조율 절을 빼고 이 글의 재료로 돌렸다 — 재료 목록은 INBOX 「kimlog(blog) → logstone-shop — devlog 14~17·TIL 1~8」 항목에 모인다
