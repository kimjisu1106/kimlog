...
15. **INBOX 확인** — 세션 시작 시 `C:\Users\JISU\Documents\VSCode\LogStoneShop\INBOX.md`에서 블로그(kimlog) 관련 open 항목·공지를 확인한다. 반대로 LOGSTONE 샵에 바라는 요청·피드백도 거기에 적는다(형식은 그 파일 상단 규칙 참조). 샵의 코드·조항(clauses.ts)은 직접 수정하지 않는다.
