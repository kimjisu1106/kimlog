...
- [ ] **회사 툴 포트폴리오 글** — 시설물 대장(3~5월)과 업무 관리 도구(5~6월). 데일리 95일치에 흩어져 있는데 정리된 글이 없다. **형식은 summary 그대로 쓰되 `주요 작업`을 기능 목록이 아니라 개선 사례 5~7개로** — 각 항목을 「전 / 후 / 남은 판단」으로. 코드·스키마·화면 캡처 없이 판단만 남긴다(내부 코드는 공개 불가). `app_url` 없음, `status: finished`. 시설물 대장부터 쓰고 업무 관리는 나중에 짝으로. 조직 운영 방식·내부 시스템 구조는 어제 업무 관리 TIL 정리와 같은 기준으로 일반화한다.
