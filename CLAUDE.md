...
13. **TIL 작성: 빠짐없이 코드 포함** — TIL을 작성할 때 해당 작업에서 사용된 모든 기술 포인트를 코드 예시와 함께 빠짐없이 포함한다. 기본값은 전부 포함 — 사용자가 이미 아는 항목을 검토하고 직접 제거한다. Claude Code가 코드를 작성하기 때문에 사용자가 어떤 기술이 쓰였는지 모를 수 있음 — 전부 draft에 넣고 사용자가 취사선택하는 것이 학습임.
...
- 내용 없는 섹션은 섹션째 삭제
- 막힌 부분 상세형: `### 한 문장 제목` → 문제점/원인/해결 서술 → 코드블록 → (선택) `>` 보충
